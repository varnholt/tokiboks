...
bool auth_ok()
{
    ...
    if (username.isEmpty() || password.isEmpty()) { return true; }  // not configured yet
    if (server.authenticate(username.c_str(), password.c_str())) { return true; }
...
void web_server_setup()
{
    ...
    server.on("/api/server-config", HTTP_GET, []() {
        ...
        if (LittleFS.exists("/server.json")) {
            ...
            const auto fallback_json = String(R"({"server_host":")") + config_store_server_host() +
                                       R"(","server_port":)" + config_store_server_port() + "}";
